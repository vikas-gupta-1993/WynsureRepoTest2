...
SET ERRORLEVEL=0
IF EXIST "%TMPSITEINFOFILE%" FOR /F %%I IN (%TMPSITEINFOFILE%) DO IF "%%I"=="%SITENAME%" SET bSITEALREADYEXIST=TRUE
%APPCMD% list apppool %APPPOOL% > %TMPSITEINFOFILE%
SET ERRORLEVEL=0
IF EXIST "%TMPSITEINFOFILE%" FOR /F %%I IN (%TMPSITEINFOFILE%) DO IF "%%I"=="%SITENAME%" SET bAPPPOOLALREADYEXIST=TRUE
