...
(function($){
  $.fn.display3DCar = function(rank,container,AllowSelection,damagecarPartsTitle,carOriginColor,carDamageColor) {
    ...
    function selectCarPart(){
      var objectToselectId;
      var checkBoxName = this.children[0].name;
      if ( checkBoxes[checkBoxName] != undefined )
        for ( objectToselectId in checkBoxes[checkBoxName]) {
          var myobject = checkBoxes[checkBoxName][objectToselectId];
          if (myobject.material.materials[0].color.getHex() == carDamageColor){
            myobject.material.materials[0].color.setHex(carOriginColor);
          }else{
            myobject.material.materials[0].color.setHex(carDamageColor);
          }
        }
    }
    function init() {
        ...
        result.scene.traverse( function ( object ) {
          if ( object instanceof THREE.Mesh ) {
            ...
            if (AllowSelection == true){
              ...
              var selector = '.deg_'+objectName.replace(/\./g, "_")+'_'+rank;
              var checkbox = $(selector);
              if (checkbox != undefined)
                checkbox = checkbox[0];
                
              if (checkbox != undefined){
                checkbox.addEventListener( 'click', selectCarPart, false );
                var checkBoxName = checkbox.children[0].name;
                if ( checkBoxes[checkBoxName] == undefined )
                  checkBoxes[checkBoxName] = [];
                checkBoxes[checkBoxName][ checkBoxes[checkBoxName].length ] = object;
              }
            }
          }
        });
      ...
      if (AllowSelection == true)
        if (firstPageX == event.pageX && firstPageY == event.pageY){
          ...
          if ( intersects.length > 0 ) {
            ...
            var selector = '.deg_'+objectName.replace(/\./g, "_")+'_'+rank;
            var checkbox = $(selector);
            var checkBoxName = $(selector)[0].children[0].name;
            var objectToselectId;
            //When a car part is select, we make all related part selected too.
            //the related car part depends of checkbox on screen ( see init part)
